...
func initRouter() {
	...
	beego.ErrorHandler("404", http404)
	beego.ErrorHandler("500", http500)

	// add template func
	beego.AddFuncMap("dateFormat", utils.Date.Format)
}

func http404(rw http.ResponseWriter, req *http.Request) {
...
func http500(rw http.ResponseWriter, req *http.Request) {
